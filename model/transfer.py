...
N_ITERS = torch.tensor([50, 40, 30])
LAMBDAS = torch.tensor([20, 5, 0.5])
MASK_RATIOS = torch.tensor([0.2, 0.1, 0])
LR = torch.tensor([10.0e-3, 3.0e-3, 1e-3])
BLUR_RADII = torch.tensor([0, 0, 0])
IMAGE_SCALES = torch.tensor([0.5, 1.0, 1.5])
RESOLUTION = 700
...
def transfer_style(style_reference_path, input_mesh_path, cfg: dict = {}):
    ...
    with torch.no_grad():
        ...
        laplace_beltrami = get_combinatorial_laplacian(orig_mesh, log_time=True)
    ...
    for lam, mask_ratio, lr, iters, blur_radius, img_scale in zip(
        ...
        for i in range(iters):
            ...
            x_hat = cholmod_solve(
                x_star, L
            )  # get vertices from implicit paramterization
            x_hat = x_hat.to(device)
            print(f"solved cholesky in {time.time() - t0} s")

            imgs: torch.Tensor = (
                render_mono_texture_from_meshes(  # construct and render mesh
                    Meshes(
...
if __name__ == "__main__":
    ...
    save_obj(
        f"data/mesh_outputs/spot_triangles.obj",
